...
class DentalCariesEnsemble:
    ...
    def evaluate_ensemble(self, test_dir, methods=['weighted_average']):
        ...
        # ===== Evaluate ConvNeXt Individually (for ensemble use only) =====
        ...
        # Store ConvNeXt results (not for graphing)
        results['convnext'] = {
        ...
        # ===== Evaluate YOLO Individually (for ensemble use only) =====
        ...
        # Store YOLO results (not for graphing)
        results['yolo'] = {
    ...
    def compare_methods(self, test_dir):
        """Compare only ensemble methods (excluding individual models from graphs)"""
        print("Comparing ensemble methods...")
        
        results = self.evaluate_ensemble(test_dir, self.ensemble_methods[:4])  # Exclude learned_weights
        
        # Create comparison DataFrame - ONLY ensemble methods
        comparison_data = []
        
        # Add ONLY ensemble methods (skip individual models)
        for method, result in results.items():
            if method not in ['convnext', 'yolo']:  # Skip individual models
                row = {
        ...
        # Plot comparison - ONLY ensemble methods
        plt.figure(figsize=(12, 6))
        
        methods = comparison_df['Method']
        accuracies = comparison_df['Overall_Accuracy']
        
        # Use different colors for ensemble methods
        colors = ['skyblue', 'lightgreen', 'gold', 'orange'][:len(methods)]
        bars = plt.bar(methods, accuracies, color=colors)
        
        plt.title('Ensemble Methods Accuracy Comparison', fontsize=14, fontweight='bold')
        plt.ylabel('Accuracy', fontsize=12)
        plt.xlabel('Ensemble Methods', fontsize=12)
        ...
        self._create_detailed_ensemble_comparison(comparison_df, results)
        
        return results, best_method
    
    def _create_detailed_ensemble_comparison(self, comparison_df, results):
        """Create a more detailed comparison chart for ensemble methods"""
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))
        
        # Left plot: Overall accuracy
        methods = comparison_df['Method']
        accuracies = comparison_df['Overall_Accuracy']
        colors = ['skyblue', 'lightgreen', 'gold', 'orange'][:len(methods)]
        ...
        # Right plot: Per-class accuracy heatmap
        per_class_data = []
        ensemble_methods = [method for method in results.keys() if method not in ['convnext', 'yolo']]
        
        for method in ensemble_methods:
            if 'per_class_accuracy' in results[method]:
                class_accs = [results[method]['per_class_accuracy'].get(cls, 0) for cls in self.classes]
                per_class_data.append(class_accs)
        
        if per_class_data:
            per_class_df = pd.DataFrame(per_class_data, 
                                      index=[m.replace('_', ' ').title() for m in ensemble_methods],
                                      columns=self.classes)
            
            sns.heatmap(per_class_df, annot=True, fmt='.3f', cmap='YlOrRd', ax=ax2)
            ax2.set_title('Per-Class Accuracy Heatmap', fontsize=14, fontweight='bold')
            ax2.set_xlabel('Classes', fontsize=12)
            ax2.set_ylabel('Ensemble Methods', fontsize=12)
        
        plt.tight_layout()
        plt.savefig('detailed_ensemble_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def create_confusion_matrices(self, results, save_dir='ensemble_results'):
        """Create confusion matrices ONLY for ensemble methods"""
        os.makedirs(save_dir, exist_ok=True)
        
        # Only create confusion matrices for ensemble methods
        ensemble_methods = [method for method in results.keys() if method not in ['convnext', 'yolo']]
        ...
        for method in ensemble_methods:
            ...
            plt.title(f'Confusion Matrix - {method.replace("_", " ").title()}\n'
                    f'Accuracy: {result["accuracy"]:.3f}', fontweight='bold')
...
def main():
    ...
    if os.path.exists(TEST_DIR):
        ...
        print(f"📈 Graphs saved: 'ensemble_methods_comparison.png' and 'detailed_ensemble_comparison.png'")
